...
- **GTK4 CSS**: structural styling lives in `style.css` at the repo root and uses only custom properties for colours and sizes; palettes live in `themes/`. Named GTK colours (`@foo`) are confined to `themes/gtk*.css`. Everything is embedded at build time and registered by `ui/theme.rs`; sizes are generated per monitor by `ui/metrics.rs`.
