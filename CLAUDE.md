...
- **e2e/** — end-to-end harness: `harness.sh` boots a nested headless niri (cage) and drives the overlay with pointer clicks and screenshots, `test.sh` is the suite, `render-themes.sh` regenerates the theme gallery (`docs/themes.md` + `docs/themes/*.png`) and `render-readme.sh` the README screenshot (`docs/readme.png`) — rerun them when `style.css` or `themes/` change; `render-lib.sh` holds what they share. See `e2e/README.md`.
