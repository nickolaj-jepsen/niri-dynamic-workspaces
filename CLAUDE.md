...
- **GTK4 CSS**: structural styling lives in `style.css` at the repo root and uses only custom properties for colours and sizes; palettes live in `themes/`. Named GTK colours (`@foo`) are confined to `themes/gtk*.css`. Everything is embedded at build time and registered by `ui/theme.rs`, which also applies `general.theme` (built-in palette or user CSS file) each time the overlay opens; sizes are generated per monitor by `ui/metrics.rs`.
