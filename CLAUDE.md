...
- **actions.rs** — workspace action choreography shared by the CLI and the overlay: niri IPC call → on-create/on-delete hooks. Column reordering and delete completion (waiting for the windows to close, then unsetting the name) run in the background under an application hold guard. `resolve_create_request` turns `switch --template/--var/--title` into the name, programs and hook context of a new workspace.
